...
export function filterClients(clients: Client[], neighborhood: string, query: string) {
  const terms = normalize(query).split(/\s+/).filter(Boolean)
  return clients.filter(client => {
    const searchable = normalize(`${client.code} ${client.name} ${client.neighborhood ?? ''}`)
    return (neighborhood === 'all' || client.neighborhood === neighborhood) && terms.every(term => searchable.includes(term))
  })
}
